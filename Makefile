...
check-git:
	...
	@if ! git remote -v | grep "upstream[[:space:]]git@github.com:ethereum/pyrlp.git (push)\|upstream[[:space:]]https://github.com/ethereum/pyrlp (push)"; then \
		echo "Error: You must have a remote named 'upstream' that points to 'pyrlp'"; \
		exit 1; \
	fi
